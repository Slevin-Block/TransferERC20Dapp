...
const JSON_File = "ERC20Token.json"
...
export const useRxWeb3 = () => {
    ...
    return { ...data, action: data?.contract?.methods }
}
...
// RESEARCH EVENT
export const researchEvent = (nameEvent, cb, blockNumber = 0) => {
...
export function watchingEvents(eventName, setData) {
  // Créez un Observable à partir de l'événement en utilisant fromEvent
  ...
  // Utilisez la méthode scan pour accumuler les data
  const data$ = event$.pipe(
    map(log => Object.fromEntries(Object.entries(log).filter(([key, value]) => isNaN(key))))
  );

  // Abonnez-vous à l'Observable pour mettre à jour les data avec setData
